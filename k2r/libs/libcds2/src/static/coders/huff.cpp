...
namespace cds_static
{
	...
	static uint countingSort(uint *_freqs, uint max_v, uint *&new_order){
		if (max_v==0) return 0;
		vector<uint> freqs(_freqs,_freqs+max_v+1);
//	  uint *freqs = new uint[max_v+1];
//	  for (uint i=0;i<=max_v;i++)
//	  	freqs[i]=_freqs[i];
		vector<uint> origFreqs(freqs.begin(),freqs.end());
//	  uint *origFreqs = new uint[max_v+1];
//	  for (uint i=0;i<=max_v;i++)
//	    origFreqs[i]=freqs[i];
		assert(origFreqs.size()==(max_v+1));

	  uint maxFreq = freqs[0];
	  ...
	  uint ndistinct = rg->rank1(rg->getLength()-1);
		vector<uint> bucket(ndistinct,0);
	  for (uint i=0;i<=max_v;i++){
	    bucket[freqs[i]]++;
	  }
	...
	bool testSorting(uint *freq, uint *newOrder, uint max_v){
		for (uint i=1;i<=max_v;i++){
			assert(freq[newOrder[i-1]]<=freq[newOrder[i]]);
		}
		return true;
	}
	...
	THuff createHuff (uint *freq, uint lim) {
		...
		countingSort(freq, lim, newOrder);
		assert(testSorting(freq, newOrder, lim));
		...
		for (i=0;i<=(int)lim;i++) {
			if (freq[i]>0) {
				tree[j].freq = freq[newOrder[i]];
				tree[j].symb = newOrder[i];
//				 tree[j].freq = freq[i];
//				 tree[j].symb = i;
				j++;
			}
...
//		sort (tree,0,lim);
	...
	size_t encodeHuff (const THuff H, uint symb, uint *stream, size_t ptr) {
		...
		while (d--) {
			if ((code >> d) & 1) bit_set(stream, ptr);
