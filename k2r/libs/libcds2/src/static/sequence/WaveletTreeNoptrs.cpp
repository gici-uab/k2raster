...
namespace cds_static
{
	...
	WaveletTreeNoptrs::WaveletTreeNoptrs(uint * symbols, size_t n, BitSequenceBuilder * bmb, Mapper * _am, bool deleteSymbols) : Sequence(n) {
		bmb->use();
		this->n = n;
		this->am = _am;
		this->am->use();
	...
	WaveletTreeNoptrs::WaveletTreeNoptrs(uint * symbols, size_t n, vector<BitSequenceBuilder*> &bsbs, Mapper * am, bool deleteSymbols) : Sequence(n) {
		for (auto &it:bsbs)
			it->use();
		this->n = n;
		this->am = am;
		...
		bitstring = new BitSequence*[height];
		for(uint i=0;i<height;i++) {
			buildBestBitmap(bsbs,_bm[i],new_n,bitstring[i]);
			delete [] _bm[i];
		}
		delete [] _bm;
	...
	size_t WaveletTreeNoptrs::range(int i1, int i2, int j1, int j2, int leftb, int rightb, int symb, int level, vector<pair<int,int> > *res, bool addRes) {
	...
	size_t WaveletTreeNoptrs::rangeCount(size_t xs, size_t xe, uint ys, uint ye){
		assert(ye>=ys);
		int yss = am->map(ys);
		int yee = am->map(ye);
	...
	void WaveletTreeNoptrs::range(int i1, int i2, int j1, int j2, vector<pair<int,int> > *res){
		assert(i2>=i1);
		int yss = am->map(j1);
		int yee = am->map(j2);
