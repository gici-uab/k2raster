...
namespace cds_utils
{
	...
	// kevin // const uint mask31 = 0x0000001F;

	/** number of bits in a uint */
	// kevin // const uint W = 32;
	/** W-1 */
	// kevin // const uint Wminusone = 31;

	/** 2W*/
	// kevin // const uint WW = 64;

	/** number of bits per uchar */
	// kevin // const uint bitsM = 8;

	/** number of bytes per uint */
	// kevin // const uint BW = 4;
	...
	/** reads bit p from e */
//	#define bitget(e,p)
    // kevin // inline uint bitget(uint *e, size_t p){
    // kevin //     return ((((e)[(p)/W] >> ((p)%W))) & 1);
    // kevin // }
    // kevin // inline uint bitget(ulong *e, size_t p){
    // kevin //     return ((((e)[(p)/WW] >> ((p)%WW))) & 1);
    // kevin // }

	/**Included in the libcdsBasics of Trees**/
	#define bitget1(e,p) ((e)[(p)/W] & (1<<((p)%W)))
	/** sets bit p in e */
	inline void bit_set(uint *e, size_t p) {
		e[p/W] |= (1<<(p%W));
	}
    inline void bit_set(ulong *e, size_t p) {
        e[p/WW] |= (1<<(p%WW));
    }

	/** cleans bit p in e */
	// kevin // inline void bitclean(uint * e, size_t p) {
	// kevin // 	e[p/W] &= ~(1<<(p%W));
	// kevin // }

    /** cleans bit p in e */
    // kevin // inline void bitclean(ulong * e, size_t p) {
    // kevin //    e[p/WW] &= ~(1<<(p%WW));
    // kevin // }
    ...
    /** uints required to represent n integers of e bits each */
    inline ulong uint_len_ulong(const ulong e, const size_t n) {
        //+((unsigned long long)e*n%W>0));
        return ((unsigned long long) e * n + W - 1) / W;
    }
...
#define bitsW 5 // OJO
// kevin // typedef unsigned char byte;
